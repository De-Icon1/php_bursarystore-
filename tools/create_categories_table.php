...
if($check && $check->num_rows > 0){
    $msg = 'Table `categories` already exists.';
} else {
    if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create'])){
        $migration = __DIR__ . '/../sql/migrations/002_create_categories.sql';
        if(!file_exists($migration)){
            ...
            if($sql === false) $errors[] = 'Failed to read migration file.';
                ...
                // Execute the SQL (single statement)
                if($mysqli->multi_query($sql)){
                    // flush
                    while($mysqli->more_results()) { $mysqli->next_result(); }
                    $msg = 'Table `categories` created successfully.';
                    if(function_exists('log_action') && isset($_SESSION['user_id'])) log_action($_SESSION['user_id'], 'Created categories table via admin tool');
                } else {
...
<!doctype html>
<html>
<head><meta charset="utf-8"><title>Create categories table</title>
<link rel="stylesheet" href="../assets/css/bootstrap.min.css"></head>
<body class="p-4">
<div class="container">
  <h4>Create `categories` table</h4>
  ...
  <?php if(empty($msg)): ?>
    <p>This will create the `categories` table. Back up your DB before proceeding.</p>
    <form method="post">
      <button name="create" class="btn btn-danger">Create categories table</button>
